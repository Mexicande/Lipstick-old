...
public class AddAddressActivity extends BaseActivity {
    ...
    private void initJsonData() {//解析数据
        ...
        ArrayList<JsonBean> jsonlsit = parseData(JsonData);//用Gson 转成实体
        options1Items = jsonlsit;
         ...
         */
        for (int i=0;i<jsonlsit.size();i++){//遍历省份
            ArrayList<String> CityList = new ArrayList<>();//该省的城市列表（第二级）
            ArrayList<ArrayList<String>> Province_AreaList = new ArrayList<>();//该省的所有地区列表（第三极）


            for (int c=0; c<jsonlsit.get(i).getCityList().size(); c++){//遍历该省份的所有城市
                String CityName = jsonlsit.get(i).getCityList().get(c).getName();
                CityList.add(CityName);//添加城市

                ArrayList<String> City_AreaList = new ArrayList<>();//该城市的所有地区列表

                //如果无地区数据，建议添加空字符串，防止数据为null 导致三个选项长度不匹配造成崩溃
                if (jsonlsit.get(i).getCityList().get(c).getArea() == null
                        ||jsonlsit.get(i).getCityList().get(c).getArea().size()==0) {
                    City_AreaList.add("");
                }else {

                    for (int d=0; d < jsonlsit.get(i).getCityList().get(c).getArea().size(); d++) {//该城市对应地区所有数据
                        String AreaName = jsonlsit.get(i).getCityList().get(c).getArea().get(d);
